...
r = requests.get(url + '/api/show/disk-groups', headers=headers, verify=False)

response = json.loads(r.content)

channels = []

for disk_group in response['disk-groups']:
    total_space = float(disk_group['size'][:-2])
    free_space = float(disk_group['freespace'][:-2])
    used_space = float(disk_group['size-numeric'])
    percentage_free = (free_space / total_space) * 100
    channel = {
        'channel': disk_group['name'] + ' - capacity',
